...
Return JSON: { "topics": [{ "topic": "title", "content": "detailed insight (2-4 sentences)" }] }`;
  ...
  try {
    ...
    // Save topics to tenant settings.knowledge (merge with existing manual topics)
    const [tenant] = await db.select().from(tenants).where(eq(tenants.id, tenantId)).limit(1);
    const settings = ((tenant?.settings || {}) as Record<string, unknown>);
    const existingKnowledge = (settings.knowledge || []) as Array<{ id: string; topic: string; content: string; autoGenerated?: boolean }>;

    // Remove old auto-generated topics, keep manual ones
    const manualTopics = existingKnowledge.filter((k) => !k.autoGenerated);

    // Add new auto-generated topics
    ...
    const mergedKnowledge = [...manualTopics, ...newTopics];

    await db.update(tenants).set({
      settings: { ...settings, knowledge: mergedKnowledge, worldModelUpdatedAt: new Date().toISOString() },
    }).where(eq(tenants.id, tenantId));

    return { success: true, topicsGenerated: newTopics.length, manualTopicsKept: manualTopics.length };
  } catch (error) {
