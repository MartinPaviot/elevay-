...
import { Mail, Plus, ChevronDown, Trash2, ExternalLink, Shield } from "lucide-react";
    ...
    <>
      ...
      {mailboxes.length > 0 && (
        <div className="mt-6 space-y-2">
          {mailboxes.map((mb) => {
            ...
            return (
              <div key={mb.id} className="rounded-lg p-4"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    {mb.status === "warming_up" && (
                      <button onClick={() => skipWarmup(mb.id)}
                        className="rounded-md px-2 py-1 text-[11px] font-medium transition-colors"
                        style={{ color: "var(--color-accent)" }}>
                        Skip warm-up
                      </button>
                    )}
                    <button onClick={() => deleteMailbox(mb.id)}
                ...
                {wp && (
                  <div className="mt-3 flex items-center gap-3">
                    <div className="h-1.5 flex-1 rounded-full" style={{ background: "var(--color-bg-muted)" }}>
      ...
      {!showSetup ? (
        <button onClick={() => setShowSetup(true)}
          className="mt-6 flex h-9 items-center gap-1.5 rounded-md px-3 text-[12px] font-medium transition-colors"
          style={{ border: "0.5px solid var(--color-border-moderate)", color: "var(--color-text-primary)", boxShadow: "var(--shadow-button)" }}>
          <Plus size={14} />
          Add account
        </button>
      ) : (
        <div className="mt-6 rounded-lg p-5" style={{ background: "var(--color-bg-surface)", border: "0.5px solid var(--color-border-default)" }}>
          {/* Pre-connection settings */}
          <div className="space-y-5">
                ...
                {CREATION_OPTIONS.map((opt) => (
                  <label key={opt.value}
                    ...
                    style={{
                      ...
                      border: contactCreation === opt.value ? "0.5px solid var(--color-accent)" : "0.5px solid transparent",
                    }}>
            ...
            <div>
              ...
              <input value={doNotTrack} onChange={(e) => setDoNotTrack(e.target.value)}
                placeholder="example.com user@example.com"
                className="mt-2 h-8 w-full rounded-md px-3 text-[12px] outline-none"
                style={{ background: "var(--color-bg-muted)", border: "0.5px solid var(--color-border-default)", color: "var(--color-text-primary)" }} />
            </div>
          </div>

          {/* OAuth connect buttons */}
          <div className="mt-6 flex items-center gap-2" style={{ borderTop: "0.5px solid var(--color-border-default)", paddingTop: "20px" }}>
            <button onClick={() => setShowSetup(false)}
              className="h-9 rounded-md px-4 text-[12px] font-medium transition-colors"
              style={{ color: "var(--color-text-secondary)", border: "0.5px solid var(--color-border-moderate)" }}>
              Cancel
            </button>
            ...
            <button disabled
              ...
              <span className="rounded px-1 py-0.5 text-[9px]" style={{ background: "rgba(255,255,255,0.15)" }}>Soon</span>
            </button>
          </div>
