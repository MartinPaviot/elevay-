...
| PROPOSAL-004 | Proofread / review UX | 003 | Low-confidence-first triage, inline edit, regenerate-per-component, export | BUILT (complete) — triage + inline edit + regenerate-per-component (with guidance) + export |
...
| PROPOSAL-006 | PDF support | 002 | Regenerate-as-PDF (zero-dep) for download; AcroForm fill/ingest needs pdf-lib | BUILT (006a regenerate: renderProposalPdf + ?as=pdf + UI, sample in _artifacts). 006b fill/ingest needs pdf-lib (Martin's env) |
| PROPOSAL-007 | Eval harness | 002 | Deterministic graders (coverage/placeholders/calibration) + key-gated scorecard CLI | BUILT (graders + scripts/eval-proposals.ts); LIVE measurement + real-Office fixtures need Martin's keys/docs |
...
| PROPOSAL-009 | Independent trust grading | 003 | Citation-support verification + TRACE-style graded confidence; knowledge citable | BUILT (complete) — grade.ts citation-support + MIN(grade,self) confidence + unsupported flag + knowledge-citable (AC3) |
