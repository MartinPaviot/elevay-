...
- R4.3b **Lists as the universal interaction (Martin, 2026-06-11).** Every multi-value criterion SHALL render as a visible list of removable tags (the legacy `Tag`/`ChipInput`/`MultiSelectDropdown` pattern): add via taxonomy search or type-and-Enter, remove via the tag's ×, current selections always visible without opening anything. This applies to every guided section (industries, sizes, geographies, exclusions, technologies, keywords, hiring titles, seniorities, person titles) AND to the Advanced disclosure, where each criterion is one list row (field · operator · tag-list value · importance) — the raw "comma, separated, values" text input SHALL NOT survive anywhere.
...
- R4.11 **Lists as the universal interaction (Martin, 2026-06-11).** Every multi-value criterion SHALL render as a visible list of removable tags (the legacy `Tag`/`ChipInput`/`MultiSelectDropdown` pattern): add via taxonomy search or type-and-Enter, remove via the tag's ×, current selections always visible without opening anything. This applies to every guided section (industries, sizes, geographies, exclusions, technologies, keywords, hiring titles, seniorities, person titles) AND to the Advanced disclosure, where each criterion is one list row (field · operator · tag-list value · importance) — the raw "comma, separated, values" text input SHALL NOT survive anywhere.
