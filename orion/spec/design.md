...
>    **DB PARTAGÉE** (celle d'Elevay, même `DATABASE_URL`), **modules COPIÉS (vendorés) dans
>    `src/`, JAMAIS importés via workspace**, **repo SÉPARÉ `@orion/web`**, **PAS** de Convex,
>    scope **tenant `elevay`** seul.
...
| D3 | **Couplage avec Elevay** | **Découplé côté code.** Orion est un repo autonome (`@orion/web`) mais **partage la DB d'Elevay** (même `DATABASE_URL`, scope tenant `elevay` via RLS) — pas de DB ni de tenant store propres. Il **n'appelle pas** l'API Elevay. Il **copie (vendore)** les modules métier sous `src/` (REUSE = copie, **jamais** import cross-repo/workspace). | Deux copies du code des gates/brief à maintenir. Accepté : c'est le prix de l'autonomie de déploiement du code (la DB, elle, reste partagée avec Elevay). |
| D4 | **Frontière de produit** | Orion = **amont seulement** : ingestion → résolution d'identité → composition par précédence → acquisition de signaux → scoring → **brief** → **export gaté**. Orion **ne rédige pas** la prose et **n'envoie pas** les colds. `draft_outreach` (prose) est **supprimé** ; remplacé par `get_outreach_brief` (intelligence structurée, zéro `subject`/`body`). | On ne capte pas la valeur « sender ». Accepté : c'est *le* pivot défendable — on devient la couche que Instantly/OrangeSlice/Lopus **consomment** (Fiber, lui, est une *source d'entrée*, pas un consommateur du brief), pas un concurrent frontal (`signal-outreach-brief:6`). Le défaut « copy vide » d'Elevay (mémoire `project_copy-quality-eval`) sort structurellement du périmètre. |
...
| D6 | **Clés partenaires** | **Clés de SINK (sortie) per-tenant, stockées en DB** (chiffrées), **JAMAIS en env** — exactement le pattern Instantly d'Elevay (pas d'`INSTANTLY_API_KEY` env ; clé passée par appel, vérifié `orion-backend-verification §6`). **Clés de SOURCE (entrée)** : per-tenant chiffré en prod, mais **env toléré pour la démo** (`FIBER_API_KEY`, `APOLLO_API_KEY` — clé de source, hors règle `integration_credentials`/sink). Les clés de *sources souveraines gratuites* (SEC/BODACC/Sirene/crt.sh/npm) n'ont pas de clé. | Un store de secrets par tenant à chiffrer/gérer. Accepté : multi-tenant SaaS l'exige de toute façon. |
| D7 | **Migrations** | **Runner custom** `scripts/apply-migrations.ts` (ledger **partagé** `__elevay_migrations`, sha256, 1 tx/fichier, idempotent) **dès J0**. `db:migrate` (drizzle-kit) câblé à `exit 1`. La DB étant **partagée** avec Elevay, on **garde le ledger `__elevay_migrations`** (PAS de journal neuf, PAS de numérotation à zéro) et on n'applique QUE des migrations **net-new additives** (`IF NOT EXISTS`), numérotées **à partir de 0107** (0106 = dernière existante) ; les `0001→0106` déjà tracées sont sautées. | Drizzle-kit `migrate` standard inutilisable. Accepté : c'est le pattern éprouvé d'Elevay. |
...
| 10 | `apps/web/scripts/apply-migrations.ts` | §2.4 — runner custom, ledger partagé `__elevay_migrations` |
...
> Le **role applicatif** est `elevay_app` (rôle restreint RLS **existant** de la DB partagée ;
> jamais de `orion_app`). Le **role opérateur** = `postgres`
...
export interface TenantSettings {
  ...
  // clés de SINK (sortie) — per-tenant, chiffrées, JAMAIS en env (D6) :
  partnerKeys?: {
    instantly?: EncryptedSecret;               // {iv, ciphertext, tag}
    orangeSlice?: EncryptedSecret;
    lopus?: EncryptedSecret;                    // webhook générique (aucune API de lecture)
    webhookSecret?: EncryptedSecret;            // secret HMAC du webhook générique
  };
  // clés de SOURCE (entrée) commerciales — per-tenant chiffré en prod ; env toléré pour la démo :
  sourceKeys?: {
    apollo?: EncryptedSecret; hunter?: EncryptedSecret; pappers?: EncryptedSecret;
    fiber?: EncryptedSecret;                    // source d'ENTRÉE (reveal + Tracker) ; FIBER_API_KEY en env OK pour la démo (clé de source)
...
```ts
// ledger PARTAGÉ avec Elevay (DB partagée) — PAS de journal neuf, on continue le sien
const TRACK = "__elevay_migrations";
const sql = postgres(process.env.DATABASE_URL_OWNER!, { max: 1 });   // role owner, hors-bande
// 1. CREATE TABLE IF NOT EXISTS __elevay_migrations (filename text PK, hash text, applied_at timestamptz default now())  -- existe déjà (Elevay)
...
CREATE TABLE IF NOT EXISTS export_jobs (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id     uuid NOT NULL,
  destination   text NOT NULL,                -- instantly|orange_slice|lopus|webhook|generic (Fiber EXCLU = source d'entrée)
...
> **Fiber-Sales EXCLU** (pas d'API d'injection). **Lopus n'est PAS une source** : c'est une
> **SORTIE** webhook générique (aucune API de lecture) — voir §6.3. Ne jamais le modéliser en
> ingesteur de signaux ni en `fiberSignalIngestor`-like.
...
export interface OutboundDestination {
  name: "instantly" | "orange_slice" | "lopus" | "webhook" | "generic"; // Fiber EXCLU (source d'entrée, jamais un sink)
...
| **Lopus** | aucune API publique → **webhook générique** (POST **HMAC-signé** de l'enveloppe brief) | secret HMAC per-tenant | NET-NEW (flatten partagé avec le webhook générique) | 0.25 |
...
| Runner migration | `scripts/apply-migrations.ts:42-103` | CLI | REUSE | ledger **partagé** `__elevay_migrations` (net-new additif ≥ 0107) |
...
| Sink signaux | `lib/signals/record-signal.ts:39,61,94` | import-direct | REUSE | `recordCompanySignal` (`:94`) ; `personFromSignals` (`:61`) |
| Taxonomie signaux | `lib/signals/taxonomy.ts` (À CRÉER ; **n'existe PAS chez Elevay**) | import-direct | **NET-NEW (prérequis dur)** | alias-map qui normalise les types bruts vers le vocab **scoring** (`SignalType`/`SIGNAL_DETECTORS` `scoring/signal-detectors.ts:16,76`, keyé par `SIGNAL_PRIORS` `scoring/signal-outcomes.ts:59`) ; type inconnu ⇒ `?? 1` ⇒ plancher 1.0× (§4.2 bug). NB : `KNOWN_SIGNAL_TYPES` (`sequences/triggers.ts:27`) est le vocab *trigger-config* — **PAS** la cible des alias. |
...
1. **Journal de migration ≠ source de vérité.** Drizzle-kit `migrate` skip les `.sql` non-journalés
   → `db:migrate` câblé `exit 1`, runner custom `apply-migrations.ts` (ledger **partagé**
   `__elevay_migrations`). DB partagée ⇒ **pas de journal neuf** : uniquement des migrations net-new
   additives (`IF NOT EXISTS`) numérotées **à partir de 0107** (0106 = dernière existante),
   ré-applications sûres.
...
11. **Split role owner/app** : migrations privilégiées via `DATABASE_URL_OWNER` (role `postgres`,
    hors-bande, 0 hit code) ; app en `elevay_app` restreint (rôle RLS **existant** de la DB partagée).
...
| **API partenaires non vérifiées** (Orange Slice endpoints, Lopus sans API de lecture) | intégration cassée | Orange Slice = flatten partagé webhook (À CONFIRMER avec clé) ; **Lopus = SORTIE webhook générique** (POST HMAC, aucune API de lecture) ; `fiberSignalIngestor` normalise tout payload d'ENTRÉE | — |
...
```
  ...
  (Orion ne rédige pas — D4)                    │  pour chaque prospect :                            │
                                                ...
                                                │     · OrangeSlice · Lopus · webhook(HMAC)            │
...
| Fiber input + export OrangeSlice/Lopus/webhook (§5.5/§6.3) | 2.0 | P1 |
