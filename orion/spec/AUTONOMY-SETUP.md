...
| **`deny` rules** | Bloque le destructeur même si allow large (deny outrank allow) | **MANQUANT** | — | Ajouter `deny` (rm -rf /, `Read(.env*)`, force-push, `--no-verify`). Bloc §2.1. NB : `DATABASE_URL_OWNER` au runtime n'est **pas** exprimable en règle `deny` (le secret n'est pas une cible `Read`/`Bash`) — c'est un invariant vérifié par `grep DATABASE_URL_OWNER src` = 0 (checklist §5 / `00-PREREQUISITES`), pas une permission committée. |
